...
class abtSponsorFlipwall { 
	...
	function shortcode_tile($atts, $content = null) {
		...
		//default image if not given
		if($image === false){
			$image = plugins_url($id.'.gif', __FILE__);
		}
		...
		ob_start();
		?>
		<div title="Click to flip" class="flipwall-tile<?php if($class != null){ echo ' ', esc_attr($class); } ?>" id="<?php echo $id?>">
			<div class="flip">
				<img alt="More about <?php echo $title?>" src="<?php echo $image?>">
			</div>
			...
			<div class="entry-meta">
				...
				<?php if( $url !== false ): ?>
				<div class="url">
					<a href="<?php echo $url?>"><?php if($linktext === false){ echo $url; } else { echo $linktext; }?></a>
				</div>
				<?php endif; //has url ?>
			</div>
